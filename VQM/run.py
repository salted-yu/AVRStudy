    ...
    def __getitem__(self, i):
        if self.file_type == "test":
            return torch.tensor(self.examples[i].input_ids), torch.tensor(
                self.examples[i].vul_query_label), torch.tensor(
...
def convert_examples_to_features(source, repair_target, tokenizer, args, language):
    ...
    for n in range(512):
        if input_ids[n] == tokenizer.start_bug_id:
            ...
            vul_query.append(0)
    return InputFeatures(input_ids, vul_query, repair_input_ids, language)
...
def test(args, model, loc_model, tokenizer, test_dataset):
    ...
    for batch in bar:
        correct_pred = False
        (input_ids, vq, repair_input_ids, languages) = [x.to(args.device) if isinstance(x, torch.Tensor) else x for x in batch]
        vul_query_mask = loc_model(input_ids)
        with torch.no_grad():
...
def main():
    ...
    tok = RobertaTokenizer.from_pretrained("../pretrain-models/codebert-base")
    tok.add_tokens(["<S2SV_StartBug>", "<S2SV_EndBug>", "<S2SV_blank>", "<S2SV_ModStart>", "<S2SV_ModEnd>", "<S2SV_Indent>"], special_tokens=True)
    encoder = RobertaModel.from_pretrained("../pretrain-models/codebert-base")
    ...
    tokenizer.add_tokens(["<S2SV_StartBug>", "<S2SV_EndBug>", "<S2SV_blank>", "<S2SV_ModStart>", "<S2SV_ModEnd>", "<S2SV_Indent>"], special_tokens=True)
