...
def evaluate(model, dataset, dataloader, tokenizer, opt):
    ...
    if opt.write_results:
        ...
        fw = open(write_path / ('{}.json'.format(opt.name + str(opt.beam_size))), 'w')
    ...
    with torch.no_grad():
        for i, batch in tqdm(enumerate(dataloader)):
            (idx, _, _, context_ids, context_mask, golden, languages) = batch
            ...
            outputs = model.generate(
                ...
                #do_sample=False, ## added beam search
                # num_beams=opt.beam_size, ## added beam search
            )
            batch_scores = []
            ...
            exactmatch.append(max(cur_score))
            ...
            if opt.write_results:
                output = {
                    ...
                    'question': example['question'],
                    'golden_answers': example['answers'],
                    'generated_answer': copy.deepcopy(answers),
                    # 'ctxs':example['ctxs']
                    'language': example['language'],
                    'score': copy.deepcopy(cur_score)
                }
                Os.append(output)
            total += 1
...
if __name__ == "__main__":
    ...
    tokenizer = AutoTokenizer.from_pretrained('./pretrain-models/codet5-base', use_fast=True, local_files_only=True)
    tokenizer.add_tokens(["<vul-start>", "<vul-end>", "<vul-sep>", '<vul-indent>'])
