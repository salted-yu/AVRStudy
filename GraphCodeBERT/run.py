...
def read_examples(filename):
    ...
    if 'reef' in filename:
        ...
        languages = ['c' for _ in range(len(srcs))]
    for i in range(len(srcs)):
    # for i in range(1145, 1150):
        examples.append(Example(source=srcs[i], target=tgts[i], language=languages[i]))
    return examples
...
def convert_examples_to_features(examples, tokenizer, args, stage=None):
    ...
    for example_index, example in enumerate(tqdm(examples, total=len(examples))):
        ...
        # deal for reef
        if args.train_filename is not None and 'reef' in args.train_filename:
            parse_lang = reef_language_dict[str(example.language).lower()]
            code_tokens, dfg = extract_dataflow(example.source,
                                                parsers[parse_lang],
                                                parse_lang)
        elif args.test_filename is not None and 'reef' in args.test_filename:
            parse_lang = reef_language_dict[str(example.language).lower()]
            code_tokens, dfg = extract_dataflow(example.source,
                                                parsers[parse_lang],
                                                parse_lang)
            ...
            code_tokens, dfg = extract_dataflow(example.source,
                                                parsers["c_sharp" if args.source_lang == "c_sharp" else "java"],
                                                "c_sharp" if args.source_lang == "c_sharp" else "java")
...
class TextDataset(Dataset):
    def __init__(self, examples, args, data_type='train'):
        self.type = data_type
    ...
    def __getitem__(self, item):
        ...
        if self.type == 'test':
            return (torch.tensor(self.examples[item].source_ids),
                    ...
                    torch.tensor(self.examples[item].target_mask), self.examples[item].language)
        else:
            return (torch.tensor(self.examples[item].source_ids),
                    torch.tensor(self.examples[item].source_mask),
                    torch.tensor(self.examples[item].position_idx),
...
def test(args, model, tokenizer, test_dataset):
    ...
    for batch in bar:
        ...
        source_ids, source_mask, position_idx, att_mask, target_ids, target_mask, languages = batch
        with torch.no_grad():
        ...
        source_input_ids = source_ids.detach().cpu().tolist()
...
def main():
    ...
    # 使用调整tokenizer之后的encoder的config
    ...
    if args.do_test:
        ...
        test_dataset = TextDataset(test_features, args, 'test')
